  ...
  while True:
    try:
      ...
      if message is not None and type(message['data']) is str:
        ...
        await redis.rpush("alerts", message['data'])

        # 새로운 메세지 유무 이벤트를 클라이언트에 emit한다.
        await sio.emit(event= event, namespace= namespace, data=True)
  ...
  while True:
    try:
      ...
      if alerts is not None:
        ...
        await sio.emit(event=event,namespace=namespace, data=alerts)
    except Exception as e:
      print(e)
      break
